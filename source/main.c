...
RGBColour ray_trace(RayDef ray, int recurse_depth) {
   ...
   for(cur_obj = 0; cur_obj < num_objs; cur_obj++){
      ...
      cur_ray.direction = vector_transform(ray.direction, matrix_transpose(object[cur_obj].transform));
   ...
   if(closest_obj != -1){
      ...
      //SurfaceNormal = vector_transform(SurfacePoint, matrix_transpose(object[closest_obj].transform));
      ToCamera = vector_scale(cur_ray.direction, -1);
         //vector_normalise(vector_subtract(cur_ray.start, SurfacePoint));
